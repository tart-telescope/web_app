<template>
  <div ref="containerRef" class="threejs-3d-container" :class="{ 'scene-visible': sceneVisible }">
...
function onMouseWheel(event) {
  ...
  const clampedSize = Math.max(ZOOM_LIMITS.min, Math.min(ZOOM_LIMITS.max, frustumSize));
...
function updateSatelliteOverlays(satelliteData, showSatellites = true) {
  ...
  for (const sat of satelliteData) {
    ...
    const ringGeometry = new RingGeometry(SATELLITE_SETTINGS.ringInnerRadius, radius, 24);
...
function createGridLines() {
  ...
  for (let az = 0; az < 360; az += 45) {
    ...
    const tubeGeometry = new TubeGeometry(curve, 40, GRID_SETTINGS.lineWidth * 0.003, 8, false);
...
function createSphereFromCorners(cornersArray) {
  ...
  sphereGeometry.setAttribute("position", new BufferAttribute(new Float32Array(vertices), 3));
  sphereGeometry.setAttribute("color", new BufferAttribute(new Float32Array(colors), 3));
  sphereGeometry.setAttribute("normal", new BufferAttribute(new Float32Array(normals), 3));
...
function animate() {
  ...
  rotationX += (targetRotationX - rotationX) * INTERACTION_DEFAULTS.smoothingFactor;
  rotationY += (targetRotationY - rotationY) * INTERACTION_DEFAULTS.smoothingFactor;
