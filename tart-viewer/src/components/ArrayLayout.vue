<template>
    <v-card class="mx-auto" flat outlined raised elevation="3" v-if="ant_sel_i">
        <v-card-title class="my-0 py-1 pr-0 teal--text text--lighten-2 text-uppercase"> Array Layout
        </v-card-title>
        <v-img aspect-ratio="1" contain>
            <svg id="overlaySVG" viewBox="0 0 512 512">
                ...
                <g class="grid">
                    ...
                    <circle v-for="ant_i in antennas" :key="ant_i.toString()" fill="white"
                        :cx="offset + scale * ant_i[0]" :cy="offset - scale * ant_i[1]" r="12" stroke-width="1"
                        stroke="rgb(0,0,0)" />
                    <text text-anchor="middle" fill="teal" dominant-baseline="middle" :x="offset + scale * 0.5"
                        :y="490">1
                        metre</text>
                    <path id='scaleLine' stroke-width='2.5' fill='none' stroke='teal' :d="scaleLine" />
                    <path id='arrow-line' marker-mid='url(#mid)' stroke-width='3' fill='none' stroke='teal' :d="line" />

                    <circle fill="white" :cx="offset + scale * ant_sel_i[0]" :cy="offset - scale * ant_sel_i[1]" :r="14"
                        style="stroke-width:3;" stroke="teal" />
                    <circle fill="white" :cx="offset + scale * ant_sel_j[0]" :cy="offset - scale * ant_sel_j[1]" :r="14"
                        style="stroke-width:3;" stroke="cyan" />
                    <text text-anchor="middle" dominant-baseline="middle" v-for="(ant_i, i) in antennas"
                        :key="i.toString() + ant_i.toString()" :x="offset + scale * ant_i[0]"
                        :y="1 + offset - scale * ant_i[1]">{{ i }}</text>
